...
void SCVars::InitCVars()
{
	...
	REGISTER_CVAR(ai_hitReactionDelay, 1.f, VF_CHEAT, "Pre attack time to give the player defend chance.");

	// Debug
	REGISTER_CVAR(ai_debug, 1, VF_CHEAT, "Debug for AI: Writes AI properties onto its.");
}
...
void SCVars::ReleaseCVars()
{
	...
	if (pConsole)
	{
		...
		pConsole->UnregisterVariable("ai_hitReactionDelay ", true);

		// Debug
		pConsole->UnregisterVariable("ai_debug", true);
	}
}